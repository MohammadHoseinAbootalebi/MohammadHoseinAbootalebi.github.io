...
Crie e implemente aplicações web avançadas usando Django. Aprenda os fundamentos da construção de um site completo utilizando Django
...
Esta especialização apresenta aos programadores Python a construção de sites utilizando a biblioteca Django. Nos quatro cursos, aprenderá HTML, CSS, SQL, Django, JavaScript, jQuery e JSON Web Services. Durante o curso, irá construir sites online utilizando Django e os sites funcionais serão avaliados ao longo do curso. Esta série é uma excelente preparação para aprender outras formas de construir aplicações web utilizando tecnologias como PHP (Web Applications for Everybody), Flask ou Ruby on Rails. Este é um excelente primeiro curso em qualquer currículo de desenvolvimento web.

Os alunos são encorajados a fazer a especialização Python for Everybody ou equivalente antes de iniciar esta especialização. Alguns podem achar útil fazer a Especialização em Web Design para Todos antes ou depois desta série.

Projeto de Aprendizagem Aplicada

Nesta especialização, os alunos aprenderão a construir aplicações Django e, no final do curso, terão construído um site simples para armazenar e visualizar anúncios classificados com funcionalidades como fotos, pesquisa e favoritos. Este site estará em produção e poderá ser adicionado ao portefólio de um aluno.

[🔗 Link do curso 🔗](https://www.coursera.org/specializations/django)
